...
export default function PrivacyScreen() {
  const { updateConsent, revokeAll, exportData, isSynced, needsVersionBump } = useConsent()
  ...
  const fieldValues: ConsentFields = {
    locationPrecise, socialFeed, socialVisibility,
    pushNotifications, marketing, analytics, surveys,
  }
    ...
    <>
      ...
      >
          ...
          {needsVersionBump && (
            <View className="mt-2 bg-amber-950 border border-amber-800 rounded-lg px-3 py-2">
              <Text className="text-amber-400 text-xs">
                ⚠️ Nossa Política de Privacidade foi atualizada para a v{CONSENT_VERSION}. Revise suas preferências abaixo.
              </Text>
            </View>
          )}
        </View>
        ...
        <View className="mx-4 mt-4 bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden">
          ...
          <View className="px-4 py-3">
            <Text className="text-xs text-zinc-400 leading-4">
              Necessários para autenticação, operação da conta e moderação de segurança. Não podem ser desativados.
            </Text>
          </View>
        </View>
        ...
        <View className="mx-4 mt-5 bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden">
          ...
          >
            <View className="flex-1">
              ...
              <Text className="text-xs text-zinc-500 mt-0.5">
                Versão {consentVersion ?? CONSENT_VERSION} · Vigência: 02/06/2026
              </Text>
            </View>
          ...
          >
            <View className="flex-1">
              <Text className="text-sm font-medium text-white">Falar com a equipe de privacidade</Text>
          ...
          >
            <Text className="text-sm font-medium text-red-400">Revogar todos os consentimentos</Text>
            <Text className="text-xs text-zinc-500 mt-0.5">
